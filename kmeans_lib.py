...
def k_means_alloc_table(all_projected_matrix, original_samples, d, n, k):
    ...
    for n in range(0, n):
        ...
        for c in range(1, k + 1):
            ...
            for s in range(1, d + 1):
                cur_dist = dist.new_distance(all_projected_matrix, all_projected_clusters, s, c)
                # cur_dist = fl.new_distance_dwt(all_projected_matrix, all_projected_clusters, s, c)
...
def create_projected_cluster(all_projected_matrix, allocation_table):
    ...
    for j in range(0, c):  # clusters
        ...
        projected_cluster = ClassFile.ProjectedSeries()
...
def cost_of_clustering(all_projected_clusters, all_projected_matrix, allocation_table):

    cost = np.zeros(4)
    for i in range(0, 4):
        for j in range(0, 4):
            if allocation_table[j][i] != 0:
                cost[i] += dist.new_distance_dwt(all_projected_matrix, all_projected_clusters,
                                               allocation_table[j][i]-1, i)
